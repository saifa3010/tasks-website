...
function stringItUp(arr){
   let string = arr.map((x)=>x.toString())
   return string
}
console.log(stringItUp([2, 5, 100]));
...
function capitalizeNames(arr){
  let Capitalize = arr.map((x)=>x.charAt(0).toUpperCase()+x.slice(1))
  return Capitalize
...
function makeStrings(arr){
  return arr.map((function(obj){
    if(obj.age >20){
      result = `${obj.name} can go to The Matrix `
      return result
    }
    else{
      result = `${obj.name} is under age!! `
      return result
    }
  }))
